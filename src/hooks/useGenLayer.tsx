...
import contractCode from '../../contracts/LendingProtocol.py?raw';
...
export interface GenTx {
  hash: string;
  type: 'deploy' | 'submit_proposal' | 'evaluate_proposal';
  proposal_id?: string;
...
export const useGenLayer = () => {
  ...
  const [contractAddress, setContractAddress] = useState<string>(
    localStorage.getItem('GENLEND_CONTRACT_ADDRESS_V2') || GLOBAL_CONTRACT_ADDRESS
  );
  ...
  useEffect(() => {
    ...
    setContractAddress(localStorage.getItem('GENLEND_CONTRACT_ADDRESS_V2') || GLOBAL_CONTRACT_ADDRESS);
    setError(null);
  ...
  const deployContract = useCallback(async () => {
    ...
    try {
      ...
      if (receipt && deployedAddress) {
          ...
          localStorage.setItem('GENLEND_CONTRACT_ADDRESS_V2', deployedAddress);
  ...
  const submitProposal = async (proposal_id: string, borrower: string, requested_amount: number, text_proposal: string) => {
      if (!contractAddress) return;
      setError(null);

      const stripGenLayerError = (msg: string) => {
          return msg.replace(/\[(?:EXPECTED|EXTERNAL|TRANSIENT|LLM_ERROR)\]\s*/ig, '');
      };
      ...
      try {
          ...
          const hash = await (client as any).writeContract({
              ...
              args: [proposal_id, borrower, requested_amount, text_proposal],
              value: BigInt(requested_amount)
          });
          ...
          setError("Failed to send submit_proposal transaction: " + stripGenLayerError(e.message));
      }
  };
  ...
  const evaluateProposal = async (proposal_id: string) => {
      ...
      try {
          ...
          setError("Consensus execution failed: " + stripGenLayerError(e.message));
      } finally {
